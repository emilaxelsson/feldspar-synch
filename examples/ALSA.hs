...
data ALSA = ALSA
    ...
    , writePCM :: PCM -> Dim1 (IArr Int16) -> Run ()
    , readPCM  :: PCM -> Data Length -> Run (Dim1 (IArr Int16))
    , closePCM :: PCM -> Run ()
    }
...
writePCM_ :: PCM -> Dim1 (IArr Int16) -> Run ()
writePCM_ pcm (Dim1 n samps) = callProc "write_pcm"
    [ objArg $ unPCM pcm
    , iarrArg samps
    , valArg n
    ]

readPCM_ :: PCM -> Data Length -> Run (Dim1 (IArr Int16))
readPCM_ pcm len = do
    samps <- newArr len
    callProc "read_pcm"
        [ objArg $ unPCM pcm
